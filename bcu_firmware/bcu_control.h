...
#define MAX_STEPS_PER_CALL 100 //TODO: Set this!
...
#define ESC_MAX_RPS 60.0
#define ESC_MIN_RPS -60.0
// 1 or -1
...
int bcu_controller_callback_states(State current, State desired, Pose acceleration){
    Input input = controller(current, desired, acceleration);
    current_inputs.left = input.left;
    current_inputs.right = input.right;
    current_inputs.ballast = input.ballast;
    update_drive();
    return EOK;
}
...
void control_text(){
    char temp[DISP_BUFFER_SIZE];
    display.setFont(FONT_BOLD);
    display.drawString(0, 0, DISP_HEADER);
    display.setFont(FONT);

    dtostrf(current_inputs.left, 2, 2, temp);
    snprintf(disp_buffer, DISP_BUFFER_SIZE, "L: %s", temp);
    display.drawString(0, 1, disp_buffer);
    dtostrf(current_inputs.right, 2, 2, temp);
    snprintf(disp_buffer, DISP_BUFFER_SIZE, "R: %s", temp);
    display.drawString(0, 2, disp_buffer);
    dtostrf(current_inputs.ballast, 2, 2, temp);
    snprintf(disp_buffer, DISP_BUFFER_SIZE, "B: %s", temp);
